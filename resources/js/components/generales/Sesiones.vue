                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">Panel de sesiones de usuarios</h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="tabla-sesiones-users" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>USUARIO</th>
                                ...
                                <tbody>
                                    <tr>
                                        <td colspan="7"> <h4 class="text-center"><img :src="ruta_img_loading" width="40px" height="40px"> Cargando registros..</h4>  </td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <tr>
...
export default {
    ...
    methods: {
        ...
        getDatosPlanDeMejoramiento: function(cirugia_id) {
            let pdm = this.sessiones_usuario.filter((valor, indice, array_old) => {
    ...
    mounted: function() {
        ...
        me.datatable_acicones = $('#tabla-sesiones-users').DataTable({
            'serverSide': true,
            ajax: {
                url: '/sesiones',
                ...
                dataSrc: function(json) {
                    let datos = json.data;
                    me.sessiones_usuario = datos;
                    return datos;
                },
            },
            columns: [
                { data: 'id' },
                { data: 'user.empleado', name: 'user.empleado.nombres', render: function(data){
                    return `${data.nombres} ${data.apellidos}`
                } },
                { data: 'user.empleado.apellidos', visible: false, },
                { data: 'user.role.nombre' },
                { data: 'inicio' },
                { data: 'finalizo' },
                { data: 'duracion', searchable: false, render: function(data){
                    let duracion = parseInt(data);
                    let hours    = ('0'+ Math.floor(duracion / 3600)).slice(-2);
                    let minutes  = ('0'+ Math.floor(duracion / 60) % 60).slice(-2);
                    let seconds  = ('0'+ duracion % 60).slice(-2);
            ...
            fnDrawCallback: function(settings, json) {
                ...
                $('table#tabla-sesiones-users > tbody > tr > td:first-child').click(function(){
                    setTimeout(() => {
                        me.activarBotonesDeAccion();
                    }, 300);
                });
        ...
        //$("div#tabla-sesiones-users_filter label").append(filtro_modulo);
        $("div#tabla-sesiones-users_filter label").append(btn_export_xlsx);
...
#tabla-sesiones-users {
    width: 100% !important;
    zoom : 80%;
}

#tabla-sesiones-users > tbody > tr {
    border-top: 5px solid #ffffff !important;
    border-bottom: 5px solid #ffffff !important;
}

#tabla-sesiones-users > thead > tr > th {
